...
on, so **Choose files** opens a folder browser in the page instead, listing
`.3mf`, `.obj` and `.stl` files. It starts at the shared folder, `./work` unless
you say otherwise, and cannot leave it. Converted files land next to their
originals, and anything Prism wrote earlier is left out of the list.
