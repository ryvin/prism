...
# what the file dialogs above accept, so the served browser offers the same
SOURCE_EXTS = ('.3mf', '.obj', '.stl')
    ...
    for n in names:
        ...
        if os.path.isdir(full):
            if _within(root, full):
                dirs.append(n)
        elif n.lower().endswith(SOURCE_EXTS) and not written.search(n):
...
_keys = []
# the server is threaded and a page load opens several requests at once
...
function drawBrowse(b){B.here=b;document.getElementById('browse').hidden=false;
 ...
 if(!b.dirs.length&&!b.files.length)list.append(el('div','none','No folders or models here.'));
